  ...
  return {
    addCity: (payload) => {
      ...
      if (_.some([name], _.isEmpty) && !isNaN(countryId)) {
        throw new Error(
          "Missing or invalid argument for adding a city [countryId, cityId, name]"
        );
      }
      ...
      if (city.length) {
        logger.warn(
          `city already exists id: ${city[0].id} name: ${city[0].name} `
        );
        return;
      }
      cities.push({ id: cityId, name });

      return { id: cityId, countryId, name };
    },
    getCountries: () => data,
    getCountryById,
...
export const { addCity, getCountries, getCountryById, getCitiesByCountryId } =
  countries();
